...
func TestPickI2PAssetWindowsPrefersJarWhenAvailable(t *testing.T) {
	t.Parallel()
...
func TestPickI2PAssetWindowsFallsBackToNativeInstaller(t *testing.T) {
	t.Parallel()
...
func TestWrapperJavaCommandWindowsUsesJavaHomeVariable(t *testing.T) {
	got := wrapperJavaCommand("windows", `C:\Program Files\Eclipse Adoptium\jre\bin\java.exe`, `C:\Program Files\Eclipse Adoptium\jre`)
	if got != `%JAVA_HOME%/bin/java.exe` {
		t.Fatalf("wrapperJavaCommand() = %q", got)
	}
}
